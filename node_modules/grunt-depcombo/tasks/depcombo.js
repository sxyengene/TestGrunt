        ...
        (function iterateFunction(pkg, name, version, callback) {
          ...
          var serialized = deptree.serialize(tree).map(function(key) {
                ...
                var projName = matches[1],
                    ...
                    filePath = matches[2] || names.slice(1).join('-') || names[0];
          ...
          if (options.output === 'url') {
            ...
            if (options.useDebug) {
              ...
              filecontent += COMBO_TEMPLATE.replace('{$url}', 'build/' + mainPkg.name.replace(/(lib|app)\./ig, '').replace('.', '-') + '.debug.js').replace('{$id}', 'main');
            } else {
            ...
            curl.get(comboUrl, function(err, response, body) {
              var mainFileContent = grunt.file.read('build/' + mainPkg.name.replace(/(lib|app)\./ig, '').replace('.', '-') + (options.useDebug?'.debug':'') + '.js')
